...
public class CodeGenerationASTVisitor extends BaseASTVisitor<String, VoidException> {
	...
	public String visitNode(CallNode n) {	//MOD (HO) - la chiamata deve usare come access link il puntatore dell'AR impacchettato nell'ID in chiamata
		...
		if(n.entry.type instanceof MethodTypeNode) {	// MOD (OO): chiamata di un metodo locale cio� dall'interno di un altro metodo di un oggetto
			return nlJoin(
	            ...
	            "ltm", 			// carico sullo stack l'Access Link (che � l'object pointer)
	            "ltm",        	// duplico il valore precedente, cio� AL / obj pointer
	            "lw",		  	// carica sullo stack l'indirizzo della dispatch table dell'oggetto (perch� l'obj pointer punta direttamente alla cella dove � memorizzato il dispatch pointer)
	            "push "+n.entry.offset, "add", // calcolo indirizzo della cella in dispatch table dove � memorizzato l'indirizzo del metodo
	...
	public String visitNode(IdNode n) {
		...
		if(n.entry.type instanceof ArrowTypeNode) { 			//MOD (HO) - carico la closure 
			return nlJoin( 
		        ...
		        "ltm", 		  // ricarico sullo stack l'indirizzo dell' AR dove � dichiarato ID
				"push "+n.entry.offset, "add", "lw", //carico l'AR dove � effettivamente dichiarata la funzione (mi servir� come Access Link nella chiamata)
				"ltm", 		  // ricarico sullo stack l'indirizzo dell' AR dove � dichiarato ID
	...
	public String visitNode(NotNode n) { 
		...
		return nlJoin(
			...
			"sub"			// la soluzione alternativa � usare beq ma richiede pi� istruzioni	
		);
	}
	...
	public String visitNode(OrNode n) { // check di ogni argomento a true
		if (print) printNode(n);
	 	...
	 	String l2 = freshLabel();
		return nlJoin(
			"push 1",		// check primo argomento - se questo � true non valuto il secondo argomento 
			visit(n.left),
			"beq "+l2,		
	...
	public String visitNode(AndNode n) {  // faccio il contrario di OR - check degli argomenti a false 
		if (print) printNode(n);
	 	...
	 	String l2 = freshLabel();
		return nlJoin(
			"push 0",		// check primo argomento - se questo � false non valuto il secondo argomento
			visit(n.left),
			"beq "+l2,		
	...
	// NOTA su AND & OR : entrambe le procedure si potrebbero implementare pi� brevi seguendo il metodo usato per NOT (con sub / add)
	// Tuttavia cos� facendo si valuterebbero sempre entrambi gli argomenti, cosa che gli altri linguaggi in genere non fanno.
	...
	@Override
	public String visitNode(MethodNode n) {		// come FunNode - per� non ritorna l'etichetta
		if (print) printNode(n,n.id);
		String declCode = null, popDecl = null, popParl = null;
	...
	public String visitNode(EmptyNode n) {
		return nlJoin("push -1"); 	//-1 � un valore diverso da qualsiasi object pointer di qualsiasi oggetto creato
	}
	...
	public String visitNode(ClassCallNode n) {	// simile a CallNode solo che occorre risalire prima al refID
		...
		return nlJoin(
			...
			"lfp", getAR, // risalita catena statica per trovare refID cio� l'object pointer ( refID pu� essere dichiarato sullo stack o essere un campo sull'heap )
            ...
            "ltm",        // duplico il valore precedente cio� AL / obj pointer
            "lw",		  // carica sullo stack l'indirizzo della dispatch table dell'oggetto (perch� l'obj pointer punta alla cella dove � memorizzato il dispatch pointer)
	...
	public String visitNode(NewNode n) {	// crea l'oggetto sull'heap caricando gli argomenti sull'heap, questi sono i campi dell'oggetto
		...
		return nlJoin(
				...
				"lhp", "sw",  	// scrivo l'indirizzo della dispatch table all'indirizzo di hp (cio� lo scrivo sull'heap subito dopo/prima dei campi)
