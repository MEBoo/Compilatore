...
public class ASTGenerationSTVisitor extends FOOLBaseVisitor<Node> {
	...
	public Node visitArrow(ArrowContext c) { // hotype di tipo arrow esempio: (hotype,hotype)=>type 
		...
		// non sono parNode come su funzione ma semplici typeNode perchè la sintassi è x:(int,int)->int e non x(a:int,b:int)->int 
	...
	public Node visitCldec(CldecContext c) { // nelle classi di FOOL i fields sono tutti dichiarati similmente ai pars delle funzioni: ClasseA (field1:int, field2:bool)
		...
		if(c.EXTENDS() != null) {
			...
			startIndex = 2;		// se la classe estende, gli ID dei fields partono dall'indice 2 anzich� dall'indice 1
		}
	...
	public Node visitDotCall(DotCallContext c) { // chiamata di un metodo: ID1.ID2(par1,pars2) - Simile a call
		...
		Node n = new ClassCallNode(c.ID(0).getText(), c.ID(1).getText(), arglist); // il primo ID � un refTypeNode (che all'interno contiene l'ID della classe a cui si riferisce)
		n.setLine(c.ID(0).getSymbol().getLine());
		return n;
	}
	
	@Override
	public Node visitNew(NewContext c) {	// simile a call (le classi di Fool non hanno costruttori, ogni field � passato come argomento con il new)
		if (print) printVarAndProdName(c);
