...
public class SymbolTableASTVisitor extends BaseASTVisitor<Void,VoidException> {
	...
	private Map<String, Map<String,STentry> > classTable = new HashMap<>(); //MOD (OO): per ogni classe dichiarata vado a memorizzare campi e metodi
																			//          serve per poter effettuare l'estensione di una classe altrimenti quando esco da ogni nl perdo le informazioni perchè rimuovo le symboltable
	...
	public Void visitNode(FunNode n) {
		...
		n.setType(new ArrowTypeNode(parTypes,n.retType));   	//MOD: setto il tipo che non � settato come per gli altri DecNode nella fase ASTGeneration 
																//     ha senso non settarlo durante l'ASTgeneration? Forse si perch� in quella fase vengono settati i tipi parsati
																//	   questo invece � un tipo derivato dall'analisi del nodo, stessa cosa vale per il tipo di una "classe"

		visit(n.getType()); 									//MOD: verifico se � coinvolto un tipo RefType e in tal caso verifico la dichiarazione della classe a cui si riferisce
		
		STentry entry = new STentry(nestingLevel,n.getType() ,decOffset);	//MOD (HO): l'offset va decrementato di 2 anzich� di 1 (ora tutte le funzioni occupano due spazi su AR)
		decOffset-=2;
		...
		for (ParNode par : n.parlist) {
			...
			if(par.getType() instanceof ArrowTypeNode) //MOD (HO): l'offset va incrementato di 2 anzich� 1 - lo pre-incremento per usare lo slot successivo e riservare quello attuale (i parametri sono positivi su AR)
				parOffset++;
	...
	public Void visitNode(ClassNode n) {
		...
		Map<String, STentry> hm = symTable.get(nestingLevel);	// nestingLevel � sempre 0 per le dichiarazioni delle classi
		...
		n.setType(type);												// instanto setto il type, verr� aggiornato qui sotto con campi e metodi
		
		STentry entry = new STentry(nestingLevel,type , decOffset--);	// la dichiarazione della classe sfrutta 1 elemento dello stack
																		// infatti l'AR dove � dichiarata la classe � sempre nel nesting-level 0 (ProgLetIn) 
																		// quindi quando si fa ClassCall si usa l'AR globale per settare l'access-link.
		
		Map<String, STentry> vt = null; 					 		 	// virtual-table - elemento da inserire in class-table (serve per permettere l'ereditariet�)
		...
		HashSet<String> locals = new HashSet<>();		// OTTIMIZZAZIONE: mi serve a verificare se localmente � gi� definito un campo/metodo 
		...
		for (FieldNode f : n.fields) {
			...
			visit(f.getType()); 						//MOD: verifico se � coinvolto un tipo RefType e in tal caso verifico la dichiarazione della classe a cui si riferisce
			
			locals.add(f.id);
		
			STentry fieldEntry = null;
			
			if(vt.containsKey(f.id)) {		// se il campo � gi� in vt allora faccio override
				...
				if (superFieldEntry.type instanceof MethodTypeNode)	// non consento overriding di un super-metodo con un campo
				{
					...
					stErrors++;
					locals.remove(f.id);	// lo tolgo per permettere la successiva ri-definizione corretta dato che al momento � stato skippato
					continue;
				}
		...
		for (MethodNode m : n.methods) {
			...
			// check overriding: se � un overriding di un NON MethodTypeNode skippo il metodo direttamente
			boolean overriding=false;
			STentry superMethodEntry = null;
			if(vt.containsKey(m.id)) {		// se il metodo � gi� in vt allora verifico se � possibile fare override
	...
	public Void visitNode(MethodNode n) {
		...
		if(hm.containsKey(n.id)) {									// overriding - il controllo circa la possibilit� � gi� fatto da ClassNode
	...
	public Void visitNode(NewNode n) {				// simile a CallNode
		...
		if( !classTable.containsKey(n.id) )			// n.id � sempre l'ID di una classe - quindi verifico direttamente in classTable
		{
